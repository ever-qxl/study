...
* 影响 ： 对共享资源的无序操作可能会带来数据的混乱，或操作错误。。
	
3. 同步互斥机制

>同步 ： 同步是一种协作关系，为完成操作，多进程或者线程间形成一种协调，按照有序的步骤执行操作

>互斥 ： 互斥是一种制约关系，当一个进程或者线程占有资源时会进行加锁，其他进程线程就无法操作该资源
...
1. 定义
>死锁是指多个线程在执行过程中，由于竞争资源或者由于彼此通信而造成的一种阻塞的现象。

2. 死锁产生条件

>死锁发生的必要条件
>>* 互斥条件：即在一段时间内某资源只由一个进程占用。
>>* 请求和保持条件：指线程已经保持至少一个资源，又提出新的资源请求，而该资源被其它进程占有，此时请求线程阻塞，但又对自己已获得的其它资源保持不放。
>>* 不剥夺条件：指线程已获得的资源，在未使用完之前，不能被剥夺
